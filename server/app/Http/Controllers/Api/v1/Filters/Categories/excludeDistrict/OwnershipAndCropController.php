...
class OwnershipAndCropController extends Controller
{
    public function filterWithOwnershipAndCrop($ownership, $crop) {
        $gradeData = $this->storageByGrade($ownership, $crop);
        $ownershipData = $this->storageByOwnership($ownership, $crop);
        $cropStoredAndCapacity = $this->storedCropAndCapacity($ownership, $crop);
        $warehouseCapacityAndCrops = $this->warehouseCapacityAndCrops($ownership, $crop);
        $utilization = $this->warehouseUtilization($ownership, $crop);
        $cropsLocation = $this->getCropsByLocation($ownership, $crop);
    ...
    public function storageByGrade($ownership, $crop) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
    ...
    public function storageByOwnership($ownership, $crop) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
        $warehouses = Warehouse::join('stock_takings', 'stock_takings.warehouse_id', '=', 'warehouses.id')
    ...
    public function storedCropAndCapacity($ownership, $crop) {
        // return $ownership;
        $collection = collect([]);
    ...
    public function warehouseCapacityAndCrops($ownership, $crop) {
    ...
    public function warehouseUtilization($ownership, $crop) {
        $collection = collect([]);
        // crops stored
        $data = StockTaking::join('districts', 'districts.id', '=', 'stock_takings.district_id')
                        ->join('regions', 'regions.id', '=', 'districts.region_id')
    ...
    public function getCropsByLocation($ownership, $crop) {
