...
class RegistrationAndGradeController extends Controller
{
    public function filterWithRegistrationAndGrade($registration, $grade) {
        $gradeData = $this->storageByGrade($registration, $grade);
        $ownershipData = $this->storageByOwnership($registration, $grade);
        $cropStoredAndCapacity = $this->storedCropAndCapacity($registration, $grade);
        $warehouseCapacityAndCrops = $this->warehouseCapacityAndCrops($registration, $grade);
        $utilization = $this->warehouseUtilization($registration, $grade);
        $cropsLocation = $this->getCropsByLocation($registration, $grade);
    ...
    public function storageByGrade($registration, $grade) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
        if($registration == 'yes') {
    ...
    public function storageByOwnership($registration, $grade) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
        if($registration == 'yes') {
    ...
    public function storedCropAndCapacity($registration, $grade) {
        // return $ownership;
        $collection = collect([]);
        if($registration == 'yes') {
    ...
    public function warehouseCapacityAndCrops($registration, $grade) {
    ...
    public function warehouseUtilization($registration, $grade) {
        $collection = collect([]);
        // crops stored
        if($registration == 'yes') {
            $data = StockTaking::join('districts', 'districts.id', '=', 'stock_takings.district_id')
    ...
    public function getCropsByLocation($registration, $grade) {
