...
{
    public function filterWithOnlyCrop($crop) {
        $gradeData = $this->storageByGrade($crop);
        $ownershipData = $this->storageByOwnership($crop);
        $cropStoredAndCapacity = $this->storedCropAndCapacity($crop);
        $warehouseCapacityAndCrops = $this->warehouseCapacityAndCrops($crop);
        $utilization = $this->warehouseUtilization($crop);
        $cropsLocation = $this->getCropsByLocation($crop);
    ...
    public function storageByGrade($crop) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
    ...
    public function storageByOwnership($crop) {
        $category = collect([]);
        $capacity = collect([]);
        $collection = collect([]);
        $warehouses = Warehouse::join('stock_takings', 'stock_takings.warehouse_id', '=', 'warehouses.id')
    ...
    public function storedCropAndCapacity($crop) {
        // return $ownership;
        $collection = collect([]);
    ...
    public function warehouseCapacityAndCrops($crop) {
    ...
    public function warehouseUtilization($crop) {
        $collection = collect([]);
        // crops stored
        $data = StockTaking::join('districts', 'districts.id', '=', 'stock_takings.district_id')
                        ->join('regions', 'regions.id', '=', 'districts.region_id')
    ...
    public function getCropsByLocation($crop) {
